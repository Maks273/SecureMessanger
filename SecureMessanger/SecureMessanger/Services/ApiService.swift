...
class ApiService {
    ...
    func addUserInContacts(id: Int, completion: @escaping (_ success: Bool?, _ error: Error?) -> Void) {
        guard let hash = CredentialManager.sharedInstance.currentUser?.hash else { return }
        ...
        AF.request(baseURL.appending("\(hash)/\(udid)/user/contacts/add"), method: .post, parameters: dict, encoding: JSONEncoding.default, headers: ["access-token": privateHeaderAccessToken]).validate().responseJSON { response in
            switch response.result {
            case .success(let result):
                if let dict = result as? [String: Any], let data = dict["data"] as? Bool {
                    completion(data, nil)
    ...
    func deleteUserFromContacts(id: Int, completion: @escaping (_ success: Bool?, _ error: Error?) -> Void) {
        guard let hash = CredentialManager.sharedInstance.currentUser?.hash else { return }
        ...
        AF.request(baseURL.appending("\(hash)/\(udid)/user/contacts/remove"), method: .post, parameters: dict, encoding: JSONEncoding.default, headers: ["access-token": privateHeaderAccessToken]).validate().responseJSON { response in
            switch response.result {
            case .success(let result):
                if let dict = result as? [String: Any], let data = dict["data"] as? Bool {
                    completion(data, nil)
    ...
    func deleteChatMember(chatId: Int, memberId: Int, completion: @escaping (_ success: Bool?, _ error: Error?) -> Void) {
        guard let hash = CredentialManager.sharedInstance.currentUser?.hash else { return }

        let udid = UIDevice.current.identifierForVendor!.uuidString.sha256()

        let dict = ["chatId": chatId, "ids": [memberId]] as! [String: Any]
        
        AF.request(baseURL.appending("\(hash)/\(udid)/chat/members/remove"), method: .post, parameters: dict, encoding: JSONEncoding.default, headers: ["access-token": privateHeaderAccessToken]).validate().responseJSON { response in
            switch response.result {
            case .success(let result):
                if let dict = result as? [String: Any], let data = dict["data"] as? Bool {
                    completion(data, nil)
    ...
    func addChatMember(chatId: Int, memberId: Int, completion: @escaping (_ success: Bool?, _ error: Error?) -> Void) {
        guard let hash = CredentialManager.sharedInstance.currentUser?.hash else { return }
        ...
        AF.request(baseURL.appending("\(hash)/\(udid)/chat/members/add"), method: .post, parameters: dict, encoding: JSONEncoding.default, headers: ["access-token": privateHeaderAccessToken]).validate().responseJSON { response in
            switch response.result {
            case .success(let result):
                if let dict = result as? [String: Any], let data = dict["data"] as? Bool {
                    completion(data, nil)
    ...
    func updateChatInfo(chatId: Int, chatName: String?, avatarFileId: Int?, completion: @escaping (_ success: Bool?, _ error: Error?) -> Void) {
        guard let hash = CredentialManager.sharedInstance.currentUser?.hash else { return }

        let udid = UIDevice.current.identifierForVendor!.uuidString.sha256()

        let dict = ["id": chatId, "name": chatName, "avatarFileId": avatarFileId] as [String : Any]
        
        AF.request(baseURL.appending("\(hash)/\(udid)/chat/update"), method: .post, parameters: dict, encoding: JSONEncoding.default, headers: ["access-token": privateHeaderAccessToken]).validate().responseJSON { response in
            switch response.result {
            case .success(let result):
                if let dict = result as? [String: Any], let data = dict["data"] as? Bool {
                    completion(data, nil)
